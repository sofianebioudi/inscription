...
function signIn($login, $motDePasse) {
    ...
    $query = mysqli_prepare($connexion, "SELECT * FROM clients WHERE login = ? AND motDePasse = ?");
    ...
    mysqli_stmt_bind_param($query, "ss", $login, $motDePasse);
    $success = mysqli_stmt_execute($query);

    if (!$success) {
        $_SESSION['error']= "Erreur d'exécution de la requête : " . mysqli_error($connexion);
        header("Location: connexion.php");
    }
